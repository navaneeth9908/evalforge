...
- Suite, output, repeated-observation, stability-policy, comparison-policy, grounding-evaluation, tool-trace, trajectory, and dataset-manifest files are untrusted input and must pass duplicate-key, strict schema, Unicode, finite-number, numeric-literal-length, size, nesting, node-count, and string-length validation. Release, stability, and regression thresholds reject coercive types; suite policies reject ambiguous legacy/declarative combinations; grounding claims require strict integer spans with complete answer coverage.
